...
document.querySelectorAll('.gallery__filters .chip').forEach(chip => {
  chip.addEventListener('click', () => {
    document.querySelectorAll('.gallery__filters .chip').forEach(c => c.classList.remove('active'));
    chip.classList.add('active');
    const label = chip.textContent.trim().toLowerCase();
    document.querySelectorAll('.gtile').forEach(t => {
      const cat = (t.dataset.cat || '').toLowerCase();
      t.style.display = (label === 'all' || cat.includes(label) || label.includes(cat)) ? '' : 'none';
    });
  });
});
...
form?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    note.hidden = false;
    note.textContent = '✓ Thank you! Your appointment request has been received. We will contact you shortly.';
    note.style.color = '';
    form.reset();
    ...
    console.error(err);
    note.hidden = false;
    note.textContent = '⚠ Could not submit. Please call us on 0120-4552014 or WhatsApp +91-92177 54696.';
